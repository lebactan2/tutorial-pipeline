...
def _configure_espeak_library() -> bool:
    ...
    # Fallback 2: search common Windows install paths.
    if platform.system() == "Windows":
        for base in (
            Path(os.environ.get("ESPEAK_NG_HOME", "")),
            Path("C:/Program Files/eSpeak NG"),
            Path("C:/Program Files (x86)/eSpeak NG"),
        ):
            lib_path = base / "libespeak-ng.dll"
            if lib_path.exists():
                try:
                    from phonemizer.backend.espeak.wrapper import EspeakWrapper

                    EspeakWrapper.set_library(str(lib_path))
    ...
    # Fallback 3: search common Homebrew/system paths on macOS
