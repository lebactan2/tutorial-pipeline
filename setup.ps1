...
Invoke-NativeChecked { uv pip install -r (Join-Path $PipelineRoot "requirements.txt") } "Core Python package install failed"
...
try {
    Set-Location $vieneuDir
    Invoke-NativeChecked { uv pip install -e . } "VieNeu editable install failed"
    Write-Log "VieNeu-TTS installed (editable mode)"
} catch {
    Write-Log "Standard install failed, trying turbo mode: $_" "WARN"
    try {
        Invoke-NativeChecked { uv pip install "vieneu[turbo]" } "VieNeu turbo install failed"
        Write-Log "VieNeu-TTS installed (turbo/pip fallback)"
    } catch {
        Write-Log "VieNeu-TTS install failed completely: $_" "WARN"
        Write-Log "Local voice generation will use the built-in Windows TTS fallback until VieNeu is installed." "WARN"
        Write-Log "To enable VieNeu voice cloning, install Visual Studio Build Tools 2022 with Desktop development with C++ and re-run setup." "WARN"
    }
}
Set-Location $PipelineRoot
...
if (Test-Path $systemConfig) {
    try {
        $existingSystemConfig = Get-Content $systemConfig -Raw | ConvertFrom-Json
    } catch {
        Write-Log "Could not read existing system.json, using setup defaults" "WARN"
    }
}
...
$config = @{
    ...
    llm_provider = if ($existingSystemConfig.llm_provider) { $existingSystemConfig.llm_provider } else { "antigravity" }
    antigravity_model = if ($existingSystemConfig.antigravity_model) { $existingSystemConfig.antigravity_model } else { "gemini-2.0-flash" }
    openrouter_model = if ($existingSystemConfig.openrouter_model) { $existingSystemConfig.openrouter_model } else { "openai/gpt-4o" }
    custom_model = if ($existingSystemConfig.custom_model) { $existingSystemConfig.custom_model } else { "" }
